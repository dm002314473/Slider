...
$(document).ready(function () {
  ...
  var animateRowNext = function animateRowNext(row) {
    ...
    $lastImageClone.prependTo(row).css({
    ...
    $lastImage.animate({
      marginRight: "-=".concat($lastImageWidth, "px")
    }, 1000);
  ...
  var animateRowPrev = function animateRowPrev(row) {
    ...
    $firstImageClone.appendTo(row).css({
      marginLeft: -$firstImageWidth + $imageMargin,
      opacity: 0
    });

    // Animating the appearance of the first image at the end of the row
    $firstImageClone.animate({
      marginLeft: $imageMargin,
      opacity: 0
    }, 500);
    $firstImageClone.animate({
